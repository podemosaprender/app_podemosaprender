...
# S: textos ################################################
def texto_edit_puede(texto,user): #U: verdadero si user puede editar este texto
	#TODO: no hardcodear usuario. habria que buscar en una lista.
	r= (
		texto.de_quien == user or
		user.username == 'melisawhatsapp' or
		user.username == 'gonzalezsolisnm'
		)
	print(f'texto_edit_puede: {r} De {texto.de_quien} User: {user}')
	return r
...
def texto_edit(request, pk=None, charla_pk=None, charla_titulo=None): #U: crear Y editar textos, de charlas o para empezar una
	...
	if not pk is None:
		...
		if not texto_edit_puede(texto, request.user):
